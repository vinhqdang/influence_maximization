...
topology) and three real social networks at increasing scale (224,
168, and 532-node Facebook ego-networks): the synthetic-graph finding
reproduces cleanly, and an initial real-network divergence traced to two
causes -- an insufficient compute budget and a harness bug that gave
one-shot baselines an unearned one-round propagation head start over
MF-BWI-Fair and a repeated-greedy baseline -- is resolved once both are
fixed, restoring MF-BWI-Fair's lead over every one-shot classical
baseline at all three real-graph scales; the sole disclosed exception is
a small, stable margin behind the more expensive repeated-greedy rollout
baseline on the largest real graph, attributed to a genuine ramp-up-speed
cost of the mean-field relaxation on richer community structure rather
than the harness artifact. We additionally benchmark our IMM
reimplementation against the original authors' own C++ code and find it
trustworthy (within 0.6\% expected spread).
\end{abstract}
...
sweeps described in Section~\ref{sec:setup}, mean $\pm$ half-width of a
95\% confidence interval over the 15 trials
(Figure~\ref{fig:main-sweeps} shows the full sweep, not just these three
points, confirming the same pattern holds in between). At $\beta=0$,
$q=0.05$ -- close to the plain progressive-IC setting every one-shot
baseline targets, where Lemma~\ref{lem:recovery-submodular} guarantees
neither MF-BWI-Fair nor repeated-greedy can exceed the proven $(1-1/e)$
ceiling regardless of $q$ -- MF-BWI-Fair and repeated-greedy are
statistically indistinguishable (paired Wilcoxon signed-rank test,
$W=49$, $p=0.53$, $n=15$); the moment either $\beta$ or $q$ departs from
its low end, MF-BWI-Fair opens a lead over every other algorithm that is
both large and statistically significant (paired Wilcoxon signed-rank
test against repeated-greedy and against the best one-shot baseline at
every other column and sweep point checked, $p<0.001$ in every case),
...
Both column-1 entries are bolded deliberately: the 0.3-point gap is well
inside the pair's own trial-to-trial noise (95\% CIs overlap almost
entirely, and the paired test above finds no significant difference),
consistent with the $(1-1/e)$-tie the theory predicts at this point --
this is a reported tie, not an unresolved discrepancy.
...
repeated-greedy's fixed 0.817 by $\alpha_{\mathrm{fair}}\approx0.5$; we
note this trend is not perfectly monotonic at every intermediate point
($\alpha_{\mathrm{fair}}=0.5$ shows a local peak of 0.863, and
$\alpha_{\mathrm{fair}}=0$ a local dip to 0.829), consistent with these
intermediate differences falling inside the 15-trial confidence intervals
rather than being a reversal of the overall trend. Total spread likewise
trends higher (105.8 to 107.6) rather than falling across the same range
-- fairness and spread are not in tension in this benchmark, though
neither quantity moves perfectly monotonically at every intermediate
$\alpha_{\mathrm{fair}}$ value tested. On the runtime axis, the
...
Every experiment above is on graphs of at most 532 nodes (the largest
real graph in Section~\ref{sec:results-multigraph}), too small to
empirically stress-test the $O(Km+n\log n)$ per-round complexity of
Theorem~\ref{thm:complexity}. We additionally measure MF-BWI-Fair's
wall-clock per-round decision time (\texttt{choose\_actions}, averaged
over 5 repeats) on Barabasi--Albert graphs from $n=100$ to $n=100{,}000$
nodes at fixed average degree $\approx6$ ($m=3$ attachments), isolating
the $n$-dependence from degree effects. Figure~\ref{fig:scaling} shows
the result: per-round time grows from 0.013s at $n=100$ to 43.8s at
$n=100{,}000$, an empirical power-law fit of $O(n^{1.30})$ over
$n\in[1000,100{,}000]$ (least-squares fit in log-log space) -- mildly
superlinear rather than the pure linear-in-$n$ behavior a
reader might expect from a casual reading of $O(Km+n\log n)$ with $m=O(n)$
at fixed average degree. This is not a violation of
Theorem~\ref{thm:complexity}, which leaves $K$ (the number of mean-field
fixed-point iterations to converge) as a separate, instance-dependent
factor rather than asserting $K=O(1)$; the most plausible explanation is
that Barabasi--Albert graphs' heavy-tailed degree distribution produces
a small number of increasingly high-degree hub nodes as $n$ grows, mildly
weakening the fixed point's contraction rate and so mildly increasing
$K$ with $n$ on this specific graph family -- a real, disclosed
effect, not a claimed $O(n)$ empirical result, and a natural target for
future characterization of $K$'s own graph-family dependence. Even so,
the measured cost remains dramatically more favorable than the classical
alternatives it is compared against in Section~\ref{sec:index}: naive
Monte Carlo KKT-greedy's $O(knRm)$ per seed set is untenable at
$n=100{,}000$ (a single seed selection would require on the order of
$k\cdot n\cdot R$ full-graph passes), while MF-BWI-Fair completes a full
per-round decision in under a minute at this scale on ordinary hardware,
with no GPU or distributed infrastructure. Raw timing data and the
generating script are at
\texttt{experiments/results\_multigraph/runtime\_scaling/} and
\texttt{experiments/run\_scaling\_benchmark.py}.
...
attributes). Full detail, including every raw per-trial result, is
reported in \texttt{experiments/MULTIGRAPH\_VALIDATION.md},
\texttt{experiments/REAL\_GRAPH\_FULLBUDGET\_FOLLOWUP.md}, and
\texttt{experiments/REAL\_GRAPH\_N3\_FOLLOWUP.md}.
...
Carlo sample counts, repeated-greedy's own 2-rollout default -- see
\texttt{experiments/\allowbreak REAL\_GRAPH\_FULLBUDGET\_\allowbreak FOLLOWUP.md})
...
a harness bug (\texttt{experiments/REAL\_GRAPH\_N3\_FOLLOWUP.md}, Section
5) under which one-shot baselines' seed sets were marked active
...
effect, not a harness artifact -- repeated-greedy's expensive per-round
joint-rollout evaluation out-ramps MF-BWI-Fair's cheaper,
mean-field-decoupled Lagrangian index on this graph's richer, more
disparate 6-community structure, a limitation worth flagging
(Section~\ref{sec:limitations}) rather than a sign the method fails
broadly. The qualitative pattern documented on \texttt{real\_facebook\_348}
-- tied at low backfire/recovery, a growing MF-BWI-Fair lead as either
increases -- holds against every one-shot baseline at all three
real-graph scales post-fix.
...
Section~\ref{sec:results-multigraph} shows the main benchmark's finding
reproduces cleanly across two further synthetic scales and a structurally
different scale-free topology, and -- once compute budget is matched to
the main benchmark and a harness bug that gave one-shot baselines an
unearned propagation head start is fixed -- against every one-shot
classical baseline on all three real social networks tested, in the same
qualitative shape (tied at low backfire/recovery, growing MF-BWI-Fair
lead as either increases). This is still only three real networks (N=3
at the level of ``real networks tested'', all SNAP ego-Facebook
subgraphs), and on the largest and sparsest of the three
(\texttt{real\_facebook\_3437}, 532 nodes, 6 detected communities)
MF-BWI-Fair trails one baseline -- repeated-greedy specifically, by a
small, stable $\sim\!2.5$--$2.7\%$ mean spread margin that the harness
fix does not change, since repeated-greedy receives the identical
correction. Two competing explanations for this residual gap are
possible: (i) a genuine control-quality gap, since repeated-greedy's
expensive per-round joint-rollout evaluation can capture
community-bridging effects that MF-BWI-Fair's cheaper, mean-field-decoupled
Lagrangian index cannot; or (ii) a parameter-uncertainty cost, since
repeated-greedy is (deliberately, as a strong-baseline design choice,
Section~\ref{sec:setup}) handed the true diffusion parameters directly
while MF-BWI-Fair only ever sees its own Bayesian posterior estimates. We
distinguish these with a direct ablation: re-running MF-BWI-Fair on
\texttt{real\_facebook\_3437} with its Bayesian tracker bypassed and its
own posterior forced to the true parameters throughout (5 trials, 6
representative $\beta$/$q$ operating points) leaves the gap to
repeated-greedy statistically unchanged (mean $-2.61\%$ with the Bayesian
posterior vs.\ $-2.81\%$ with true parameters; the ablation, if anything,
trends in the opposite direction from what the uncertainty-cost
explanation would predict). This rules out explanation (ii) and is
direct, if not exhaustive, evidence for (i): the residual gap is a
genuine control-quality effect of the mean-field-decoupled index rather
than a cost of not knowing the true parameters, on graphs with richer,
more disparate community structure, rather than the mean-field
approximation degrading with density or degree per se (the smaller,
denser \texttt{real\_facebook\_348} and \texttt{real\_facebook\_686}
graphs show no such gap against any baseline, including repeated-greedy).
Raw data for this ablation is at
\texttt{experiments/results\_multigraph/real\_facebook\_3437\_true\_param\_ablation.csv}.
Broader real-network coverage, degree-distribution families beyond
ego-Facebook subgraphs, and a more direct ablation of MF-BWI-Fair's
ramp-up speed against repeated-greedy's rollout depth remain future work.
...
structurally different topology. On three real social networks tested at
increasing scale, the finding initially did not hold, for two distinct,
disentangled reasons: a necessarily reduced compute budget, and a harness
bug that gave one-shot baselines an unearned one-round propagation head
start over the two sequential methods. Fixing both restores MF-BWI-Fair's
lead over every one-shot classical baseline at all three real-graph
scales; the sole disclosed exception is a small, stable margin behind a
more expensive repeated-greedy rollout baseline on the largest, most
community-structured real graph, attributable to a genuine ramp-up-speed
cost of the mean-field relaxation rather than to either fixed artifact --
an open question for future work along with broader real-network
coverage beyond the ego-Facebook family tested here. We also directly
validated our IMM reimplementation against the original authors' C++ code
and found it trustworthy.

\section*{Code and Data Availability}
All code (simulator, algorithm and baseline implementations, experiment
drivers), raw per-trial results underlying every table and figure in this
paper, and the extended validation notes referenced throughout
(\texttt{experiments/*.md}) are publicly available at
\url{https://github.com/vinhqdang/influence_maximization}. The real-graph
datasets are the SNAP ego-Facebook subgraphs~\cite{mcauley2012learning},
themselves publicly available from the Stanford Network Analysis Project.
