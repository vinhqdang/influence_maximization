...
reproduces cleanly. On the real networks, we first observe a divergence
from this pattern, and we trace it to two causes: an insufficient compute
budget, and a harness bug that gave one-shot baselines an unearned
one-round propagation head start over MF-BWI-Fair and a repeated-greedy
baseline. Fixing both restores MF-BWI-Fair's lead over every one-shot
classical baseline at all three real-graph scales. One exception remains,
and we disclose it directly: on the largest real graph, MF-BWI-Fair
trails the more expensive repeated-greedy rollout baseline by a small,
stable margin, which we attribute to a genuine ramp-up-speed cost of the
mean-field relaxation on richer community structure rather than to the
harness artifact. We additionally benchmark our IMM
...
does not depend on this one hand-tuned graph.
...
repeated-greedy statistically unchanged: the mean gap is $-2.61\%$ with
the Bayesian posterior and $-2.81\%$ with true parameters, and if
anything trends in the opposite direction from what the
uncertainty-cost explanation would predict. This ablation rules out the
second explanation and gives direct, if not exhaustive, support for the
first: the residual gap is a genuine control-quality effect of the
mean-field-decoupled index on graphs with richer, more disparate
community structure, not a cost of not knowing the true parameters and
not the mean-field approximation degrading with density or degree per
se (the smaller, denser 348-node and 686-node graphs show no such gap
against any baseline, including repeated-greedy). Broader real-network
coverage, degree-distribution families beyond
...
one-shot classical baseline at all three real-graph scales. One
exception remains: on the largest, most community-structured real graph,
MF-BWI-Fair trails a more expensive repeated-greedy rollout baseline by
a small, stable margin, which we attribute to a genuine ramp-up-speed
cost of the mean-field relaxation rather than to either artifact we
fixed. Closing that gap, and testing on real networks beyond the
ego-Facebook family used here, remain open for future work. We also
directly validated our IMM reimplementation against the original
authors' C++ code and found it trustworthy.
