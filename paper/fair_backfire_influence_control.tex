...
classical baseline at all three real-graph scales, with one exception: on
the largest real graph, MF-BWI-Fair trails the more expensive
repeated-greedy rollout baseline by a small, stable margin (Section~\ref{sec:limitations}).
We additionally benchmark our IMM
...
\draw[->] (I) to[bend left=18] node[above, align=center, font=\scriptsize]
  {$u\in N_{\mathrm{in}}(v)$ active,\\positive trial ($p_{uv}$)} (A);
\draw[->] (A) to[bend left=18] node[below, align=center, font=\scriptsize]
...
\draw[->] (index) -- (fill);
\draw[->] (fill) -- (env);
\draw[->] (env.south) |- ++(0,-0.5) -| (bayes.south) node[midway, below, yshift=-0.5cm] {next round};
\end{tikzpicture}
...
\texttt{small\_sbm} to single digits on the three real graphs), while the
repeated-greedy comparison stays within a few percent of parity
everywhere except the two most disparately-structured graphs
(\texttt{real\_facebook\_3437} and \texttt{large\_community\_sbm}), where
it is MF-BWI-Fair that trails slightly -- visually summarizing the same
scale/structure-dependent picture developed above and in
Section~\ref{sec:limitations}.
...
mean-field-decoupled index, not a cost of not knowing the true
parameters (the smaller, denser 348-node and 686-node graphs show no
such gap against any baseline, including repeated-greedy, ruling out a
simple density- or degree-driven explanation too).
