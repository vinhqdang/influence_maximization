...
backfire-capable) independent-cascade model.

Per-round node transition, exactly as specified:

  inactive v: activates w.p. 1 - prod_{u in active in-neighbors}(1 - p_plus[u,v]),
              UNLESS a CONVERT action was applied to v this round (forced active,
              and no stochastic trial is drawn / observed for v this round).
  active v:   deactivates w.p. 1 - (1-q[v]) * prod_{u in active in-neighbors}
              (1 - p_minus[u,v]), UNLESS a MAINTAIN action was applied to v this
              round (forced to remain active, no trials drawn / observed).
...
def simulate_step(
    ...
    """Advance the diffusion by exactly one round.
    ...
    for v in G.nodes():
        ...
        if not active:
            ...
            for u in G.predecessors(v):
                ...
                if success:
                    became_active = True
            new_state[v] = became_active
        else:
            if act == Action.MAINTAIN:
                new_state[v] = True
                continue
            recovered = bool(rng.random() < q[v])
            ...
            for u in G.predecessors(v):
                ...
                p = beta * p_plus[(u, v)]
                success = bool(rng.random() < p)
                observations.append({"param": "p_minus", "u": u, "v": v, "success": success})
                if success:
                    deactivated = True
            new_state[v] = not deactivated
